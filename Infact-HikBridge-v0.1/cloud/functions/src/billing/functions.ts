...
export const activateManualSubscription = onCall({ region: "asia-south1" }, async (request) => {
  const auth = requireAuthentication(request.auth as AuthContext | undefined);
  requirePlatformAdmin(auth);
...
export const listPlatformSubscriptions = onCall({ region: "asia-south1" }, async (request) => {
  const auth = requireAuthentication(request.auth as AuthContext | undefined);
  requirePlatformAdmin(auth);
